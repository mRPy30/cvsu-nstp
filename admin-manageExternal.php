...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['activityID']) && isset($_POST['deleteInstructor'])) {
        // Delete existing external
        $activityID = $_POST['activityID'];

        // Delete the external from the database
        $deleteQuery = "DELETE FROM tbl_activities WHERE id = '$activityID'";
        $conn->query($deleteQuery);
    
        // Redirect back to the page to refresh the instructor list
        header('Location: admin-externalList.php');
        exit();
        
    } elseif (isset($_POST['updateInstructor'])) {
        // Retrieve form data
        $activityID = $_POST['instructorID'];
